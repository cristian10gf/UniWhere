...
usage() {
    cat <<'EOF'
Uso: ./run-parallel-colmap.sh <serie1> [serie2 ...] [opciones] [-- args_extra_colmap]

Ejecuta COLMAP en paralelo sobre multiples series con control de concurrencia.
Cada serie se procesa via run-series.sh en modo advanced (sin reconstruccion densa).

Opciones:
  --max-parallel N    Maximo de ejecuciones simultaneas (default: 1)
  --mode MODE         Modo de COLMAP: automatic|advanced (default: advanced)
  --data-root PATH    Carpeta base de datos (default: preprocesamiento/data)
  --with-dense        Incluir reconstruccion densa (omitida por defecto)
  --cpu               Forzar modo CPU
  -h, --help          Mostrar esta ayuda

Ejemplos:
  ./run-parallel-colmap.sh serie-1 serie-2 serie-3
  ./run-parallel-colmap.sh serie-1 serie-2 --max-parallel 2
  ./run-parallel-colmap.sh serie-1 serie-2 --mode advanced -- --max-features 16384
EOF
}

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
PROJECT_ROOT=$(realpath "${SCRIPT_DIR}/../..")
RUN_SERIES_SH="${SCRIPT_DIR}/../models/colmap/docker/run-series.sh"
...
if [ ! -x "$RUN_SERIES_SH" ]; then
    echo "Error: no se encontro run-series.sh en '${RUN_SERIES_SH}'."
    exit 1
fi

# Validate all series exist before starting
...
run_colmap_for_serie() {
    local serie="$1"
    local log="${DATA_ROOT}/${serie}/colmap.log"

    local args=("$serie" --mode "$MODE" --data-root "$DATA_ROOT")

    if [ "$FORCE_CPU" -eq 1 ]; then
        args+=(--cpu)
    fi

    if [ "$MODE" = "advanced" ] && [ "$WITH_DENSE" -eq 0 ]; then
        args+=(-- --no-dense)
    ...
    echo "[$(date +%H:%M:%S)] Iniciando COLMAP: $serie (log: $log)"
...
if [ ${#failed_series[@]} -gt 0 ]; then
    ...
    for serie in "${failed_series[@]}"; do
        echo "  - $serie (ver ${DATA_ROOT}/${serie}/colmap.log)"
    done
fi
